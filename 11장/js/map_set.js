...
//set() 메서드를 여러번 쓰기 귀찮을떄, 연결해서 사용할 수 있다.
...
console.log(numSet3); //증복되는 값을 제거하고 셋을 만듬.
//값만 나온다
console.log(numSet1.keys()); //키가 없으므로 값이 출력
console.log(numSet1.values());
console.log(numSet1.entries()); //값:값으로 출력

//values 메서드를 사용해 값을 가져와 for of 반복문으로
//lanfuage 셋에 있는 값을 출력하시오.
let language=new Set(['js','c','python','c','js']);

for (let a of language.values()) {
    console.log(a);
}
